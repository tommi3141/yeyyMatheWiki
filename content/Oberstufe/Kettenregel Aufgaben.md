...
>> 
>> Erforderliches Grundwissen für diese Aufgabe: Kettenregel
>> Wir schreiben f(x) als Verkettung zweier Funktionen 
>> $$
>> f(x) = u(v(x))
>> $$
>> - Äußere Funktion $u(x)$: 
>> $$
>> u(x) = \frac{1}{x} = x^{-1} \quad \Rightarrow \quad u'(x) = -x^{-2} = -\frac{1}{x^2}
>> $$
>> - Innere Funktion $v(x)$: 
>> $$
